...
mod tests {
    ...
    #[tokio::test]
    async fn test_ingress_edge_cases() {
        ...
        let ingress_router = Arc::new(IngressRouter::new("mock://localhost").await.unwrap());
        ...
        tokio::spawn(async move {
            ...
            let ir1 = ingress_router.clone();
            let svc = service_fn(move |req| {
                handle_request(req, p1.clone(), e1.clone(), r1.clone(), ir1.clone())
            });
            let _ = http1::Builder::new().serve_connection(io, svc).await;

            // Accept one more connection for the next test
            let (stream, _) = listener.accept().await.unwrap();
            let io = TokioIo::new(stream);
            let p2 = provider.clone();
            let e2 = executor.clone();
            let r2 = registry.clone();
            let ir2 = ingress_router.clone();
            let svc = service_fn(move |req| {
                handle_request(req, p2.clone(), e2.clone(), r2.clone(), ir2.clone())
            });
            let _ = http1::Builder::new().serve_connection(io, svc).await;
        });
        ...
        // Edge Case 1: Malformed JSON Handshake
        let res = client
            .post(format!("http://{}/api/attest", addr))
            .body("not valid json")
            .send()
            .await
            .unwrap();
        ...
        // Edge Case 2: Invalid keys length (triggers Forbidden due to unsupported format)
        let body = serde_json::json!({
            "client_random": hex::encode([1u8; 32]),
            "client_challenge": hex::encode([2u8; 48]),
            "ecdhe_public": hex::encode([3u8; 1]), // Invalid length
            "mlkem_public": hex::encode([4u8; 1]) // Invalid length
        });
        ...
        let res2 = client
            ...
            .await
            .unwrap();
        assert_eq!(res2.status().as_u16(), 403); // Forbidden
    }
}
